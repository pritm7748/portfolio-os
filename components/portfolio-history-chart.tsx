...
type Props = {
    data: ChartDataPoint[]
    currentValue?: number // <--- Critical Prop for Net Worth fix
...
export default function PortfolioHistoryChart({ data, currentValue, onRangeChange, onCategoryChange, isLoading, category }: Props) {
  ...
  const chartData = useMemo(() => {
      ...
      if (activeRange === 'all') startDate = new Date(data[0].date)

      // 1. Filter by Date
      const filtered = data.filter(d => new Date(d.date) >= startDate)
      if (filtered.length === 0) return []

      // 2. Scale "Value" line to match Current Net Worth
      // We assume growth happened proportionally to investment over time
      ...
      return filtered.map(point => {
          ...
          return {
              ...point,
              value: point.invested * growthRatio // Creates the smooth curve ending at correct value
          }
      })
    ...
    if (active && payload && payload.length) {
      return (
        <div className="bg-white dark:bg-slate-900 p-3 border border-slate-200 dark:border-slate-800 rounded-lg shadow-lg text-sm min-w-[150px]">
          <p className="font-bold text-slate-700 dark:text-slate-300 mb-2 border-b border-slate-100 dark:border-slate-800 pb-1">
            {new Date(label).toLocaleDateString(undefined, { dateStyle: 'medium' })}
          </p>
          <div className="space-y-1">
              <div className="flex items-center justify-between gap-4">
                <span className="text-slate-500 text-xs">Est. Net Worth:</span>
            ...
            <ResponsiveContainer width="100%" height="100%">
                <AreaChart data={chartData} margin={{ top: 10, right: 10, left: 0, bottom: 0 }}>
                    ...
                    <XAxis 
                        ...
                        minTickGap={40} 
                        tickFormatter={(str) => { const d = new Date(str); return activeRange === '1mo' ? `${d.getDate()}/${d.getMonth()+1}` : d.toLocaleDateString('en-IN', { month: 'short', year: '2-digit' }) }} 
                    />
                    ...
                    <Area type="monotone" dataKey="value" name="Net Worth" stroke={color} fill={`url(#${gradientId})`} strokeWidth={2} />
                    <Area type="stepAfter" dataKey="invested" name="Invested" stroke="#94a3b8" fill="transparent" strokeWidth={2} strokeDasharray="4 4" activeDot={false} />
                </AreaChart>
            </ResponsiveContainer>
        )}
      </div>
