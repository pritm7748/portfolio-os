...
import { Bell, ChevronDown, User, Settings, LogOut, HelpCircle, CreditCard, Loader2 } from 'lucide-react'
...
export default function Header() {
  ...
  // Market Search State (Keep your existing search state code here...)
  // ... (keeping previous search logic hidden for brevity, assume it's there) ...
  ...
  useEffect(() => {
    const getUserAndNotifications = async () => {
      ...
      if (user) {
        ...
        // 2. Fetch Notifications (Triggered Alerts)
        const { count, error } = await supabase
            .from('price_alerts')
            .select('*', { count: 'exact', head: true }) // Count only
            .not('triggered_at', 'is', null) // Filter: triggered_at IS NOT NULL
  ...
  // ... (keep handleSignOut and getTitle functions) ...
  ...
  const getTitle = () => {
    ...
    if (pathname.includes('/alerts')) return 'Notifications' // <--- ADD THIS
    return 'Dashboard'
  }


  return (
    <header className="flex h-20 items-center justify-between border-b border-slate-200 bg-white px-8 shadow-sm dark:border-slate-800 dark:bg-slate-900 font-sans relative z-30">
      ...
      <div className="flex items-center gap-6">
        {/* (Keep Market Search logic here if you had it) */}
        ...
        {/* ... (Keep User Dropdown logic exactly the same) ... */}
        <div className="relative">
            <button 
                onClick={() => setIsDropdownOpen(!isDropdownOpen)}
                className="flex items-center gap-3 rounded-full border border-slate-200 bg-white p-1 pr-4 hover:bg-slate-50 transition-colors dark:border-slate-700 dark:bg-slate-900 dark:hover:bg-slate-800"
            >
