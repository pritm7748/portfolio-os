...
import { Plus, Search, Download, Loader2, ChevronRight, Scissors, Info, Bell, ArrowUpDown, ChevronDown } from 'lucide-react'
...
type Holding = {
    ...
    pnlPercent: number
    assetIds: number[]
    firstBuyDate: number // New for sorting
    lastBuyDate: number  // New for sorting
}
...
const SORT_OPTIONS = [
    ...
    { label: 'Highest Gain (Overall %)', value: 'pnl-desc' },
    { label: 'Biggest Loser (Overall %)', value: 'pnl-asc' },
    { label: 'Highest Gain (Today %)', value: 'day-desc' },
    { label: 'Biggest Loser (Today %)', value: 'day-asc' },
    { label: 'Latest Bought', value: 'latest-bought' },
    { label: 'First Bought', value: 'first-bought' },
]
...
export default function HoldingsPage() {
        ...
        transactions.forEach(txn => {
            ...
            if (!map[key]) {
                map[key] = {
                    ...
                    firstBuyDate: 0, lastBuyDate: 0
                }
            }

            // Track Buy Dates for Sorting
            if (txn.date && txn.transaction_type === 'Buy') {
                const tDate = new Date(txn.date).getTime()
                if (map[key].firstBuyDate === 0 || tDate < map[key].firstBuyDate) map[key].firstBuyDate = tDate
                if (map[key].lastBuyDate === 0 || tDate > map[key].lastBuyDate) map[key].lastBuyDate = tDate
            }
    ...
    const sortedAndFilteredHoldings = useMemo(() => {
        ...
        switch (sortBy) {
            ...
            case 'pnl-desc': result.sort((a, b) => b.pnlPercent - a.pnlPercent); break;
            case 'pnl-asc': result.sort((a, b) => a.pnlPercent - b.pnlPercent); break;
            case 'day-desc': result.sort((a, b) => b.dayChangePercent - a.dayChangePercent); break;
            case 'day-asc': result.sort((a, b) => a.dayChangePercent - b.dayChangePercent); break;
            case 'latest-bought': result.sort((a, b) => b.lastBuyDate - a.lastBuyDate); break;
            case 'first-bought': result.sort((a, b) => a.firstBuyDate - b.firstBuyDate); break;
        }

        return result
    }, [holdings, filterType, searchQuery, sortBy])
    ...
    return (
      <div className="space-y-6 pb-20">
            ...
            <div className="flex flex-col sm:flex-row gap-3 w-full xl:max-w-2xl">
                ...
                <div className="relative w-full sm:max-w-[220px]">
                    ...
                    <select 
                        ...
                        className="w-full appearance-none rounded-lg border border-slate-300 bg-white py-2 pl-9 pr-8 text-sm focus:border-indigo-500 focus:outline-none dark:bg-slate-950 dark:border-slate-700 dark:text-white cursor-pointer shadow-sm"
                    >
        ...
        {sortedAndFilteredHoldings.length === 0 ? (
            <div className="flex h-60 flex-col items-center justify-center text-slate-400 bg-white dark:bg-slate-900 rounded-xl border border-dashed border-slate-300 dark:border-slate-700">
