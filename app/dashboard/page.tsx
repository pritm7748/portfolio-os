...
import { ArrowUpRight, ArrowDownRight, Wallet, PieChart as PieIcon, IndianRupee, Loader2, PiggyBank, Gem, TrendingUp, DollarSign, ChevronRight } from 'lucide-react'
...
export default function DashboardPage() {
  ...
  useEffect(() => {
    const fetchData = async () => {
      ...
      try {
        ...
        let totalIncome = 0
        let dividendCount = 0
        let realizedPnL = 0 // Total Realized P&L
        ...
        // Calculate Current Holdings (FIFO Basis)
        ...
        if (tickersArray.length > 0) {
            const [priceRes, divRes] = await Promise.all([
                fetch('/api/prices', { method: 'POST', body: JSON.stringify({ tickers: holdingTickers }) }),
        ...
        tickersArray.forEach(ticker => {
            ...
            dividends.forEach((div: any) => {
                ...
                if (qtyOnDate > 0) {
                    const amount = qtyOnDate * div.amount
                    totalIncome += amount
                    dividendCount++
                }
            })
        ...
        // Buckets Calculation
        const equity = { invested: 0, current: 0 }
        const commodity = { invested: 0, current: 0 }
        ...
        const calcStats = (inv: number, curr: number) => {
            const unrealized = curr - inv
            const pct = inv > 0 ? (unrealized / inv) * 100 : 0
            return { invested: inv, current: curr, unrealizedPnl: unrealized, pnlPercent: pct }
        }

        const eqStats = calcStats(equity.invested, equity.current)
        const commStats = calcStats(commodity.invested, commodity.current)
  ...
  return (
    <div className="space-y-8">
      ...
      {/* 1. GRAND TOTALS (Hero Section) */}
      <div className="grid gap-4 md:grid-cols-4"> {/* Changed to 4 columns */}
         ...
         <div className="rounded-xl bg-indigo-600 p-6 text-white shadow-lg dark:bg-indigo-700">
            ...
            <div className="mt-4 flex items-center gap-2 text-indigo-100 text-sm">
                <span className={data.total.unrealizedPnl >= 0 ? 'text-green-300' : 'text-red-300'}>
                    {data.total.unrealizedPnl >= 0 ? '+' : ''}₹{data.total.unrealizedPnl.toLocaleString('en-IN')}
                </span>
         ...
         <div className="rounded-xl border border-slate-200 bg-white p-6 shadow-sm dark:bg-slate-900 dark:border-slate-800">
            ...
            <p className="text-xs text-slate-400 mt-1">Booked Profits/Losses</p>
         </div>

         {/* Dividend Income */}
         <div className="relative rounded-xl border border-slate-200 bg-white p-6 shadow-sm dark:bg-slate-900 dark:border-slate-800 transition hover:border-emerald-200 dark:hover:border-emerald-800 group">
            <div className="flex items-center justify-between mb-2">
                ...
                {/* THE ARROW LINK */}
                <Link href="/dashboard/dividends" className="p-1 text-slate-400 hover:text-emerald-600 hover:bg-emerald-50 rounded-full transition dark:hover:bg-emerald-900/20">
