...
export default function NewsPage() {
  ...
  const getHeader = () => {
      ...
      if (selectedTicker.startsWith('SEARCH:')) return { title: `Search Results: "${selectedTicker.replace('SEARCH:', '')}"`, subtitle: 'Custom news feed.' }
      return { 
    ...
    <div className="flex flex-col lg:flex-row h-[calc(100vh-100px)] gap-6 relative">
        ...
        <div className="w-full lg:w-80 flex-shrink-0 flex flex-col bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-xl overflow-hidden shadow-sm">
            ...
            <div className="flex-1 overflow-y-auto p-2 space-y-1">
                ...
                <button
                    onClick={() => { setSelectedTicker('GENERAL'); setCustomSearch('') }}
                ...
                {/* 2. SMART FEED BUTTON */}
                <button
                    onClick={() => { setSelectedTicker('ALL'); setCustomSearch('') }}
                ...
                {holdings.map(h => {
                    ...
                    return (
                        <div key={h.ticker} className={`group flex items-center justify-between px-3 py-2 rounded-lg transition-colors ${selectedTicker === h.ticker ? 'bg-slate-100 dark:bg-slate-800' : 'hover:bg-slate-50 dark:hover:bg-slate-800/50'}`}>
                            <button 
                                onClick={() => { setSelectedTicker(h.ticker); setCustomSearch('') }}
        ...
        {/* RIGHT: MAIN FEED + SEARCH */}
        <div className="flex-1 flex flex-col min-w-0">
            
            {/* Header + Search Bar Area */}
            <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 mb-6">
                <div>
                    <h2 className="text-2xl font-bold text-slate-900 dark:text-white">{headerInfo.title}</h2>
                    <p className="text-sm text-slate-500 dark:text-slate-400">{headerInfo.subtitle}</p>
                </div>
                
                {/* GLOBAL SEARCH BAR (Moved Here) */}
                <form onSubmit={handleSearch} className="relative w-full md:w-80">
                    <input 
                        type="text" 
                        placeholder="Search news (e.g. US Inflation)..." 
                        value={customSearch}
            ...
            <div className="flex-1 overflow-y-auto">
                {newsLoading ? (
                    ...
                    <div className="grid gap-4 md:grid-cols-2 xl:grid-cols-2 pb-10">
